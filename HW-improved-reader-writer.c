...
#include "HW-improved-reader-writer.h"
...
void space(int s) {
    Sem_wait(&print_lock);
    printf("   %d    %d    %d    %d\t",rwlock.AR,rwlock.AW,rwlock.WR,rwlock.WW);   // a[]의 정보를 반영해서 헤딩 라인을 출력
    int i;
    for (i = 0; i < s * 3; i++)
        printf("\t");
}
...
int main(int argc, char *argv[]) {
    ...
    printf("   AR   AW   WR   WW   ");   // a[]의 정보를 반영해서 헤딩 라인을 출력
    for(int i = 0;i<num_workers;i++){
        if(a[i].job_type == 0)
